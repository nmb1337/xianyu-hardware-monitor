...
点击“保存设置”，再点击“发送测试”。收到 QQ 私聊后再继续。填写多个接收 QQ 时，消息会逐个发送；个别接收人发送失败只重试未成功的人，已收到的不会重复。
...
4. 第一轮扫描只建立基线，不对开始监控前已存在的商品提醒；之后程序按规则顺序连续查询，新发布且价格位于规则区间内的商品才会提醒。
5. 如果频繁出现“访问验证/登录失效”，且换浏览器、换账号都无效，通常是网络出口 IP 被风控：在“闲鱼浏览器”中把“网络出口”切换为“直连（不使用代理）”（国内宽带/移动 IP 通常更安全），或填写另一条代理线路；保存后点“关闭浏览器”再点“打开登录”。
